#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# file: crocoddyl_ddp_vfi_abs_only.py
"""
Dual-arm DQ + Crocoddyl demo — **Absolute-pose only** with Vector Field Inequalities (VFI)
(no relative-pose constraint; no relative-pose cost).

WHAT CHANGED (vs. crocoddyl_ddp_vfi.py)
- Remove relative-pose equality freeze (no `J_rel u = 0` projection).
- Remove relative-pose costs/terms from the DDP model; keep only absolute-pose terms.
- Keep VFI obstacle avoidance, now unconstrained (no Null(J_rel) restriction).
- Main loop: directly DDP -> VFI projection -> integrate.

Run: `python crocoddyl_ddp_vfi_abs_only.py`
...
class LowLevelModule:
    """Legacy container for models and joint states."""

    def __init__(self, config: ConfigModule, desire_abs_pose, desire_rel_pose=None, nullspace_gain: float = 0.8):
        self.rel_gain = float(getattr(config, "rel_gain", 0.0))
        self.abs_gain = float(getattr(config, "abs_gain", 1.0))
        self.nullspace_gain = float(np.clip(nullspace_gain, 0.0, 1.0))

        # Only absolute pose is relevant now
        self.desire_rel_pose = DQ(desire_rel_pose).normalize() if desire_rel_pose is not None else DQ(1)
...
class DQKino:
    ...
    # relative (8) — kept for completeness but unused in costs
    def rel_vec8(self, q):
...
class DiffKinoDQ(croco.DifferentialActionModelAbstract):
    ...
    State/velocity cost (absolute only):
      0.5*( (g_abs - abs(q))^T W_abs (g_abs - abs(q)) )
    Velocity shaping (absolute only):
    ...
    def set_goals(self, g_abs=None):
        if g_abs is not None:
            self.g_abs = np.asarray(g_abs, dtype=float).reshape(8)

    def set_weights(self, w_abs=None, w_u=None, w_vabs=None, beta_abs=None):
    ...
    def calcDiff(self, data, x, u=None):
        ...
        eps = 1e-6
        data.Lxx = Ja.T @ (self.w_abs_vec[:, None] * Ja) \
...
class CrocoMPC:
    def __init__(self, dq_kino: DQKino, dt=0.05, N=30, weights=None):
        ...
        if weights is None:
            weights = dict(w_abs=np.ones(8) * 1e3, w_u=5e-2, w_vabs=5e2, beta_abs=4.0)
    ...
    def set_goals(self, g_abs):
        for m in self.running_models:
            m.set_goals(g_abs=g_abs)
        self.terminal_model.set_goals(g_abs=g_abs)

    def set_weights(self, w_abs=None, w_u=None, w_vabs=None, beta_abs=None):
        for m in self.running_models:
            m.set_weights(w_abs, w_u, w_vabs, beta_abs)
    ...
    def step(self, q0, g_abs, max_iters=80, init_reg=1e-4, v_max=None):
        self._build_once(q0)
        self.problem.x0 = np.asarray(q0, dtype=np.float64).reshape(self.nq)
        self.set_goals(g_abs=g_abs)
...
class VFIProjector:
    ...
    def project(self, q, u_des, v_limit=None):
...
def main(show_bullet=True, loop_playback=False):
    ...
    llm = LowLevelModule(config, desire_abs_pose, desire_rel_pose=desire_rel_pose, nullspace_gain=0.0)
    ...
    print("Start sim (VFI, absolute-only):")
    for k in range(steps):
        q = np.concatenate([q1, q2])

        # 1) high-level from DDP (absolute only)
        u_ddp = mpc.step(q, g_abs=g_abs, max_iters=200, v_max=v_max)
        u_ddp = np.clip(u_ddp, -v_max, v_max)

        # 2) VFI projection (no equality constraints)
        u_cmd, n_active = vfi.project(q, u_ddp, v_limit=v_max)
