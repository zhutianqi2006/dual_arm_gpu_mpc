...
class MPPI_Module():
    ...
    def init_collision_model(self):
        ...
        self.curobo_robot_file = "dual_panda_curobo_obstacle.yml"
        ...
        self.init_obstacle_x = self.curobo_config.world_model.world_model.cuboid[4].pose[0]
    ...
    def update_curobo_world_model(self, time_elapsed:float):
        ...
        self.curobo_config.world_model.world_model.cuboid[4].pose[0] = self.init_obstacle_x + math.sin(0.15*time_elapsed)
        self.ros_module.write_obstacle(self.curobo_config.world_model.world_model.cuboid[4].pose[0:3])
        ...
        self.current_obstacle_x = self.curobo_config.world_model.world_model.cuboid[4].pose[0]
        self.fake_obstacle_x = self.curobo_config.world_model.world_model.cuboid[4].pose[0]

    def update_fake_curobo_world_model(self, vel:float):
        self.fake_obstacle_x += vel*self.mppi_dt
        self.curobo_config.world_model.world_model.cuboid[4].pose[0] = self.fake_obstacle_x
        self.curobo_fn.update_world(self.curobo_config.world_model.world_model)

    def update_obstacle_velocity_estimate(self):
        self.current_obstacle_x_velocity = (self.current_obstacle_x - self.last_obstacle_x)/self.mppi_dt
    ...
    def mppi_worker(self):
        ...
        for i in range(self.mppi_T):
            ...
            self.update_fake_curobo_world_model(self.current_obstacle_x_velocity)
            ...
            if i == 0:
                ...
                # print("batch_abs_position", batch_abs_position[0])
        ...
        end_time = time.time()
        print("collision_cost time", end_time-start_time)
        ...
        w_epsilon = self.moving_average_filter(w_epsilon, int(self.mppi_T/2.0))
    ...
    def traditional_control_result(self):
        ...
        for i in range(self.mppi_T):
            if i == 0:
                ...
                energy = abs_cost + abs_position_cost + d_new
            else:
                energy += abs_cost + abs_position_cost + d_new
    ...
    def warm_up(self):
        for i in range(10):
            self.update_curobo_world_model(0.0)
