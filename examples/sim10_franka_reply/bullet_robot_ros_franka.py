...
class DualArmBulletModel(Node):
    ...
    def pyb_load_environment(self,client_id):
        ...
        # 创建球体视觉形状（可选，但建议添加以可视化）
        sphere_visual_shape = pyb.createVisualShape(
            pyb.GEOM_SPHERE,
            radius=sphere_radius,
            rgbaColor=[1,0,0,0.9],  # 红色，完全不透明
            physicsClientId=client_id
        )
        cube4_id = pyb.createMultiBody(
            baseMass=0,  # 质量为0表示静态物体
            baseCollisionShapeIndex=sphere_collision_shape,
            baseVisualShapeIndex=sphere_visual_shape,
            basePosition=[0.50, 0.0, 100],  # 初始位置与原来相同
            physicsClientId=client_id
        )
        cube5_id = pyb.createMultiBody(
            baseMass=0,  # 质量为0表示静态物体
            baseCollisionShapeIndex=sphere_collision_shape,
            baseVisualShapeIndex=sphere_visual_shape,
            basePosition=[0.50, 0.0, 100],  # 初始位置与原来相同
