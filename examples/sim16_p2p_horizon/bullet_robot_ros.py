...
class DualArmBulletModel(Node):
    ...
    def pyb_load_environment(self, client_id):
        ...
        dual_arm_robot_id = pyb.loadURDF(
            'model/dual_arm_model/dual_arm_model.urdf', [0, 0, 0], useFixedBase=True, physicsClientId=client_id
        )
        dual_arm_robot = pyb_utils.Robot(dual_arm_robot_id, client_id=client_id)
        pyb.resetDebugVisualizerCamera(
            cameraDistance=1.2, cameraYaw=89, cameraPitch=-28, cameraTargetPosition=[-0.0, 0.0, 0.0]
        )
    ...
    def _save_and_cleanup(self) -> None:
        try:
            path = self.recorder.save("p2p_h15")
