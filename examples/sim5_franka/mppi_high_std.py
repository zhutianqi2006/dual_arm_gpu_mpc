...
from utils.mppi_high_std import MPPINoNullHighStdModule
# 
import rclpy
import array

class MPPINoNullHighStdModuleDynamic(MPPINoNullHighStdModule):
    def init_collision_model(self):
        ...
        self.init_obstacle_x = self.curobo_config.world_model.world_model.cuboid[2].pose[1]
        self.init_obstacle_x_dim = self.curobo_config.world_model.world_model.cuboid[2].dims[1]
    ...
    def update_obstacle_velocity_estimate(self):
        self.current_obstacle_x_velocity = (self.current_obstacle_x - self.last_obstacle_x)/0.15
...
def main(args=None):
    ...
    mppi_module = MPPINoNullHighStdModuleDynamic(config, desire_abs_pose, desire_abs_position, desire_rel_pose, desire_line_d, desire_quat_line_ref)
    mppi_module.warm_up()
    while True:
