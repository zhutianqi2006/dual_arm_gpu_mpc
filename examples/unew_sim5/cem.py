...
class CEMModuleDynamic(CEMModule):
    """CEM controller with dynamic obstacle updates (match unew_sim5 MPPI dynamic setup)."""

    def _init_collision_model(self) -> None:
        super()._init_collision_model()

        # obstacle: cuboid[2] moves along y (pose[1])
        self.init_obstacle_y = self.curobo_config.world_model.world_model.cuboid[2].pose[1]
        self.init_obstacle_y_dim = self.curobo_config.world_model.world_model.cuboid[2].dims[1]
    ...
    def update_curobo_world_model(self, time_elapsed: float) -> None:
        ...
        self.curobo_config.world_model.world_model.cuboid[2].dims[1] = self.init_obstacle_y_dim
        self.curobo_config.world_model.world_model.cuboid[2].pose[1] = self.init_obstacle_y + math.sin(0.15 * time_elapsed)
        ...
        self.current_obstacle_y = self.curobo_config.world_model.world_model.cuboid[2].pose[1]
        self.fake_obstacle_y = self.current_obstacle_y

    def update_obstacle_velocity_estimate(self) -> None:
        self.current_obstacle_y_velocity = (self.current_obstacle_y - self.last_obstacle_y) / 0.20

    def update_fake_curobo_world_model(self, vel: float) -> None:
        fake_obstacle_y_dim = 1.4 * abs(vel) * self.dt * self.T
        self.fake_obstacle_y += 0.5 * vel * self.dt * self.T
        self.curobo_config.world_model.world_model.cuboid[2].pose[1] = self.fake_obstacle_y
        self.curobo_config.world_model.world_model.cuboid[2].dims[1] += fake_obstacle_y_dim
        self.curobo_fn.update_world(self.curobo_config.world_model.world_model)
        self.curobo_fn2.update_world(self.curobo_config.world_model.world_model)
    ...
    def play_once(self) -> None:
        ...
        self.update_fake_curobo_world_model(self.current_obstacle_y_velocity)
...
def run_variance_mode(module: CEMModule, args: argparse.Namespace) -> None:
    ...
    for run_idx in range(args.runs):
        ...
        if hasattr(module, "update_fake_curobo_world_model"):
            module.update_fake_curobo_world_model(getattr(module, "current_obstacle_y_velocity", 0.0))
