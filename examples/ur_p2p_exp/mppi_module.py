...
class MPPI_Module():
    ...
    def init_collision_model(self):
        self.curobo_world_file = "ur_p2p_exp1_env.yml"
    ...
    def mppi_worker(self):
        ...
        robot1_acc_explore_seq2, robot2_acc_explore_seq2 = epsilon_generator_log(int(self.batch_size*5/100), self.robot1_q_num, self.robot2_q_num, self.mppi_T,
                                                                               self.mean, self.std/10.0, self.log_std, self.mppi_dt*self.max_acc_abs_value, self.mppi_seed)
        robot1_acc_explore_seq =torch.cat((robot1_acc_explore_seq1, robot1_acc_explore_seq2), dim=0)
        robot2_acc_explore_seq =torch.cat((robot2_acc_explore_seq1, robot2_acc_explore_seq2), dim=0)
        ...
        stagnation_cost = 10*abs(self.first_batch_fake_robot1_q-self.batch_fake_robot1_q).sum(dim=1, keepdim=True)+10*abs(self.first_batch_fake_robot2_q-self.batch_fake_robot2_q).sum(dim=1, keepdim=True)
...
def main(args=None):
    ...
    # desire_abs_pose = [- 0.009809, - 0.700866, - 0.008828, 0.713171, 0.043587, - 0.000795, - 0.272602, - 0.003556]
