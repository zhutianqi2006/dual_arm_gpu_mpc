h_fig = figure('Name', 'dual_arm_abs_refer_plot');
time_line = 9;

% Find the index where any of the first 8 columns is non-zero
non_zero_index = find(any(out.abs_refer.Data(:,1:8) ~= 0, 2), 1, 'first');

% Adjusting data to start plotting from the first non-zero index
time_data = out.abs_refer.Data(non_zero_index:end, time_line);
time_start  = out.abs_refer.Data(non_zero_index, time_line);
data_columns = out.abs_refer.Data(non_zero_index:end, 1:8);
