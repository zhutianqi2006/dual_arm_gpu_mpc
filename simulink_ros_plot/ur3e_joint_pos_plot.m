h_fig = figure('Name', 'ur3_joint_pos_plot');
time_line = 7;

% Find the index where any of the first 6 columns is non-zero
non_zero_index = find(any(out.ur3_joint_pos.Data(:,1:6) ~= 0, 2), 1, 'first');

% Adjusting data to start plotting from the first non-zero index
time_data = out.ur3_joint_pos.Data(non_zero_index:end, time_line);
time_start  = out.ur3_joint_pos.Data(non_zero_index, time_line);
data_columns = out.ur3_joint_pos.Data(non_zero_index:end, 1:6);

% Plotting each joint posocity
for i = 1:6
...
ylabel('posocity (rad/s)', 'FontSize', 20, 'Interpreter', 'tex');
