h_fig = figure('Name', 'ur3e_joint_vel_plot');
time_line = 7;

% Find the index where any of the first 6 columns is non-zero
non_zero_index = find(any(out.ur3e_joint_vel.Data(:,1:6) ~= 0, 2), 1, 'first');

% Adjusting data to start plotting from the first non-zero index
time_data = out.ur3e_joint_vel.Data(non_zero_index:end, time_line);
time_start  = out.ur3e_joint_vel.Data(non_zero_index, time_line);
data_columns = out.ur3e_joint_vel.Data(non_zero_index:end, 1:6);
