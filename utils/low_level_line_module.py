...
from dqrobotics.robot_modeling import DQ_SerialManipulatorDH,DQ_SerialManipulatorMDH, DQ_CooperativeDualTaskSpace, DQ_Kinematics
...
class LowLevelLineModule():
    ...
    def get_u(self):
        ...
        dual_arm_abs_jacobian = self.cpu_dq_dual_arm_model.absolute_pose_jacobian(self.dual_arm_joint_pos)
        desire_line_d = DQ([0, 0, 0, 1])
        line_jacobian = DQ_Kinematics.line_jacobian(dual_arm_abs_jacobian, self.dual_arm_abs_feedback, desire_line_d)
        desire_quat_line_ref = DQ([0, -0.994308, 0.00868, 0.106191])
        current_line = self.dual_arm_abs_feedback.P()*desire_line_d*(self.dual_arm_abs_feedback.P().conj())
        pinv_line_jacobian = np.linalg.pinv(line_jacobian)

        u_line = 0.000 * pinv_line_jacobian[:,:4]@(vec4(desire_quat_line_ref) - vec4(current_line)) 
        self.dual_arm_joint_vel = dual_arm_rel_joint_vel  + np.matmul(np.eye(self.robot1_q_num+self.robot2_q_num)-dual_arm_rel_jacobian_roboust_inv@(dual_arm_rel_jacobian), self.high_level_u+ u_line)
