...
class LowLevelRelChangeModule():
    def __init__(self, config: ConfigModule, desire_abs_pose, desire_rel_pose):
        # --- thread safety (why: runtime key updates may race with control loop) ---
        ...
        self.robot1_base = DQ(config.robot1_base).normalize()
        self.robot1_effector = DQ(config.robot1_effector).normalize()
        ...
        self.robot2_base = DQ(config.robot2_base).normalize()
        self.robot2_effector = DQ(config.robot2_effector).normalize()
    ...
    # --- public API: runtime update desire_rel_pose ---
    def set_desire_rel_pose(self, new_rel_pose_vec8):
        """Update desired relative pose at runtime.
        Args:
            new_rel_pose_vec8: length-8 array-like representing a DQ.
        """
        with self._lock:
            self.desire_rel_pose = DQ(new_rel_pose_vec8).normalize()

    def get_desire_rel_pose_vec8(self):
        with self._lock:
            return vec8(self.desire_rel_pose)
    ...
    def get_u(self):
        ...
        # read desired pose under lock (why: avoid tearing while keyboard updates)
        target_rel_vec8 = self.get_desire_rel_pose_vec8()
        dual_arm_rel_error = target_rel_vec8 - dual_arm_rel_feedback
        ...
        dual_arm_rel_joint_vel = 3*self.rel_gain * np.matmul(dual_arm_rel_jacobian_roboust_inv, (dual_arm_rel_error))
