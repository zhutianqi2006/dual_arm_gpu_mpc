...
from dq_torch import rel_abs_pose_rel_abs_jac
...
class MPPIBiasedStdModule():
    ...
    def mppi_worker(self):
        ...
        for i in range(self.mppi_T):
            ...
            if i == 0:
                rel_pos, bacth_abs_pos, bacth_rel_jacobian, bacth_abs_jacobian, batch_abs_position, batch_angle = rel_abs_pose_rel_abs_jac(self.gpu_robot1_dh_mat, self.gpu_robot2_dh_mat,
            ...
            batch_robot2_ith_dq = batch_robot2_ith_dq + biased_vel[:,self.robot1_q_num:]
...
@torch.jit.script
def get_abs_error(desire_abs_pos:torch.Tensor, abs_pos:torch.Tensor):
    abs_error = desire_abs_pos - abs_pos
    return abs_error.unsqueeze(-1)
