...
from dq_torch import rel_abs_pose_rel_jac, project_q_by_constraint_jacobian
...
class MPPINewInvModule():
    ...
    def mppi_worker(self):
        ...
        robot1_acc_explore_seq, robot2_acc_explore_seq = epsilon_generator(int(self.batch_size), self.robot1_q_num, self.robot2_q_num, self.mppi_T,
                                                                               self.mean, self.std, self.mppi_dt*self.max_acc_abs_value, self.mppi_seed)
        ...
        for i in range(self.mppi_T):
            ...
            self.batch_fake_robot1_q, self.batch_fake_robot2_q, batch_robot1_ith_proj_dq, batch_robot2_ith_proj_dq=update_joint_position_with_limits(self.batch_fake_robot1_q, self.batch_fake_robot2_q, 
                                        batch_robot1_ith_dq, batch_robot2_ith_dq,
                                        self.gpu_robot1_q_min, self.gpu_robot1_q_max, 
                                        self.gpu_robot2_q_min, self.gpu_robot2_q_max, self.mppi_dt)
            last_batch_robot1_mppi_dq = batch_last_robot1_mppi_result[:,i,:]
            last_batch_robot2_mppi_dq = batch_last_robot2_mppi_result[:,i,:]
            torch.cuda.synchronize()
            start_time = time.time()
            batch_proj_robot1_q, batch_proj_robot2_q = project_q_by_constraint_jacobian(self.gpu_robot1_dh_mat, self.gpu_robot2_dh_mat,
                                             self.batch_robot1_base,  self.batch_robot2_base, 
                                             self.batch_robot1_effector, self.batch_robot2_effector, 
                                             self.batch_fake_robot1_q, self.batch_fake_robot2_q, self.batch_desire_rel_pose,
                                             self.robot1_q_num, self.robot2_q_num, self.robot1_dh_type, self.robot2_dh_type)
            # call kernel...
            torch.cuda.synchronize()
            print("projection time:", time.time() - start_time)
            batch_robot1_ith_proj_dq = (batch_proj_robot1_q-self.last_batch_fake_robot1_q)/self.mppi_dt
            batch_robot2_ith_proj_dq = (batch_proj_robot2_q-self.last_batch_fake_robot2_q)/self.mppi_dt
            robot1_acc_explore_seq[:,i,:self.robot1_q_num] = batch_robot1_ith_proj_dq- last_batch_robot1_mppi_dq
            robot2_acc_explore_seq[:,i,:self.robot2_q_num] = batch_robot2_ith_proj_dq- last_batch_robot2_mppi_dq
            ...
            self.batch_fake_robot1_q, self.batch_fake_robot2_q, batch_robot1_ith_proj_dq, batch_robot2_ith_proj_dq=update_joint_position_with_limits(self.last_batch_fake_robot1_q, self.last_batch_fake_robot2_q, 
...
def compute_weights(
    ...
    if not torch.isfinite(S).all():
        raise RuntimeError("S contains NaN/Inf.")
    if not torch.isfinite(epsilon).all():
        raise RuntimeError("epsilon contains NaN/Inf.")
    if S.numel() != batch_size:
        raise RuntimeError("batch_size != S.numel().")
    ...
    # 3) 稳定归一化：先减最小值，再用 logsumexp
    rho = torch.min(S)                   # 标量
    a = (-1.0 / p) * (S - rho)           # [B]
    log_eta = torch.logsumexp(a, dim=0)  # 标量
    w = torch.exp(a - log_eta)           # [B]，且 sum(w)=1

    # 4) 兜底：若仍出现非有限数，退化为 one-hot 在 argmin(S)
    if not torch.isfinite(w).all():
        w = torch.zeros_like(S)
        min_idx: int = int(torch.argmin(S).item())
        w[min_idx] = 1.0

    # 5) 按你原来的形状约定做广播（[B,1,1]）
